...
def parse_versions(obj: Any) -> list[str]:
    vals = sorted({s for s in walk_strings(obj) if VERSION_RE.match(s)})
    return vals


def version_dt(s: str) -> pd.Timestamp:
    return pd.Timestamp(datetime.strptime(s, "%Y%m%d-%H%M"), tz=HKT)
...
class ArchiveClient:
    def __init__(self, sleep_s: float = 0.15):
        ...
        self.s.headers.update({
            "User-Agent": "polymarket-weather-phase2/1.0",
    ...
    def list_versions(self, start: str, end: str) -> tuple[list[str], str]:
        data, base = self.get_json("list-file-versions", {
            "url": RESOURCE_URL,
            "start": start,
            "end": end,
        })
        return parse_versions(data), base
...
def main():
    ...
    ap.add_argument("--out", default="phase1_full_out/hk_phase2_hko.csv")
    ap.add_argument("--cache", default="phase1_full_out/hko_archive_cache")
    ap.add_argument("--decision-hour", type=int, default=8)
    ap.add_argument("--max-dates", type=int, default=0, help="Smoke test: only first N unique dates")
    args = ap.parse_args()
    ...
    for i, date_s in enumerate(dates, 1):
        ...
        try:
            versions, base = client.list_versions(prev, curr)
        ...
        by_date[date_s] = rec
        print(
            f"  [{i}/{len(dates)}] {date_s} version={rec.get('hko_archive_version')} "
            f"max={rec.get('hko_forecast_max_c')} err={rec.get('hko_archive_error')}",
            file=sys.stderr,
        )
    ...
    feats = out.apply(
        lambda r: bucket_features(r.get("bucket"), r.get("hko_forecast_max_c")), axis=1, result_type="expand"
    )
    ...
    print("===== PHASE 2A HKO ARCHIVE =====")
    print(f"Rows: {len(out)}")
    print(f"Events: {out.event_slug.nunique()}")
    print(f"Dates with HKO forecast: {unique_dates_covered}/{len(dates)}")
    if "hko_archive_error" in out:
        errs = out.loc[out["hko_archive_error"].notna(), ["date_hkt", "hko_archive_error"]].drop_duplicates()
        if not errs.empty:
            print("\nArchive errors:")
    ...
    economic_summary(out, covered & (out["hko_bucket_distance_c"] <= 1.0), "HKO within 1C")
