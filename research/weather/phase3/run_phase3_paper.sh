...
python -m py_compile weather_forward_paper.py phase3_status.py

mkdir -p phase3_out

echo "===== PHASE 3 SMOKE ====="
python -u weather_forward_paper.py --once

echo
echo "===== STARTING 7/24 PAPER DAEMON ====="
tmux kill-session -t "$SESSION" 2>/dev/null || true
tmux new-session -d -s "$SESSION" "bash -lc 'cd $DIR && source ../phase1/.venv/bin/activate && python -u weather_forward_paper.py --interval 60 2>&1 | tee -a phase3_paper.log'"
sleep 3
...
echo "Status:   cd $DIR && source ../phase1/.venv/bin/activate && python phase3_status.py"
