...
def test_gated_whale_copy_relaxes_score_only_for_sampling_profile():
    ...
    assert gated_copy.threshold == pytest.approx(0.38)
...
def test_gated_whale_copy_spread_and_liquidity_relaxation_does_not_change_sampling():
    ...
    assert gated_copy.should_trade is True
    assert gated_copy.inputs["effective_max_spread_pct"] == pytest.approx(0.088, rel=1e-4)
    assert gated_copy.inputs["effective_min_volume_24h"] == pytest.approx(6750.0)
